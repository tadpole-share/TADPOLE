...
```{r univariate ADAS13 to Dementia}
CASESNL <- NULL
j=1
...
for (j in 1:length(VISIT_TRAINCROSSMRI) )
{
  ...
  CASESNL <- rbind(CASESNL,cbind(DADAS13,timetolast,tmpss[,sampledcolumns]))
}

hist(CASESNL$DADAS13)
CASESNL <- CASESNL[complete.cases(CASESNL),]
CASESNL$ICV <- NULL
#CASESNL$ADAS13 <- NULL
CASESNL$timetolast2 <- CASESNL$timetolast*CASESNL$timetolast
CASESNL$timetolast3 <- log(CASESNL$timetolast)
CASESNL$timetolast_ST34CMVO <- CASESNL$ST34CMVO*CASESNL$timetolast
TADPOLE_predictors <- read.delim("PredictorsList.txt")
...
names(timename) <- colnames(TADPOLE_predictors)
TADPOLE_predictors <- rbind(t(timename),TADPOLE_predictors)
timename <- c("timetolast_ST34CMVO","timetolast_ST34CMVO")
names(timename) <- colnames(TADPOLE_predictors)
TADPOLE_predictors <- rbind(t(timename),TADPOLE_predictors)
...
for ( repp in 1:numberofRepp)
{
  ...
  ids_ca <- unique(as.character(CASESNL$RID))
    ...
    trainTime_ADAS13 <- subset(CASESNL,RID %in% sampleIDS_TimeADAS13)
    testTime_ADAS13 <- subset(CASESNL,!(RID %in% sampleIDS_TimeADAS13))
    ...
    if (compute)
    {
      ...
      save(sys_times,file=paste("sys_times_Time_ADAS13_NL",numberOfTrials,j,repp,".RDATA",sep="_"))
      save(Time_ADAS13,file=paste("Time_ADAS13_NL",numberOfTrials,j,repp,".RDATA",sep="_"))
    }
    else
    {
      load(file=paste("Time_ADAS13_NL",numberOfTrials,j,repp,".RDATA",sep="_"))
    ...
    for (n in 1:60)
    {
        ...
        testWithAD_s$timetolast_ST34CMVO <- testWithAD_s$ST34CMVO*log(timetolast)
    ...
    for (n in 1:60)
    {
        ...
        testD3$timetolast_ST34CMVO <- testD3$ST34CMVO*log(timetolast)
...
write.csv(allforcasttestPredicts,file=paste(repp,"D2_ADAS13_NL.csv",sep="_"))
write.csv(allD3predicts,file=paste(repp,"D3_ADAS13_NL.csv",sep="_"))
...
write.csv(mean50CI,file=paste(repp,"D2_ADAS13_NL_mean50CI.csv",sep="_"))
...
write.csv(mean50CI,file=paste(repp,"D3_ADAS13_NL_mean50CI.csv",sep="_"))
...
bpstaty <- bp$stats[3,]
plot(bpstaty~bpstatx,main="All Tests",xlim=c(-5,15),ylim=c(-5,15))
