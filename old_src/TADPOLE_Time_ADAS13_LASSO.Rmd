  ...
  ids_ca <- unique(as.character(CASESAD$RID))
    ...
    if (compute)
    {
      for (n in 1:numberOfTrials)
      {
        ...
        singlecaseTime_ADAS13$BDX <- NULL;
      ...
      save(sys_times,file=paste("sys_times_Time_ADAS13",numberOfTrials,j,repp,".RDATA",sep="_"))
      save(Time_ADAS13,file=paste("Time_ADAS13",numberOfTrials,j,repp,".RDATA",sep="_"))
    }
    else
    {
      load(file=paste("Time_ADAS13",numberOfTrials,j,repp,".RDATA",sep="_"))
    ...
    foldenet <- try(glmnet::cv.glmnet(as.matrix(trainTime_ADAS13[,-1]),as.vector(trainTime_ADAS13[,1]),family="gaussian"));

#    mp <- medianPredict(as.character(baggedformula),trainTime_ADAS13,testTime_ADAS13,predictType ="linear",type="LM")
    mplasso <- predict(foldenet,as.matrix(testTime_ADAS13[,-1]))
#    alltestpredicts <- rbind(alltestpredicts,cbind(testTime_ADAS13$DADAS13,mplasso))
    plot(testTime_ADAS13$DADAS13~mplasso)
    ct <- cor.test(testTime_ADAS13$DADAS13,mplasso)
    logitcor <- append(logitcor,ct$estimate)
    res <- testTime_ADAS13$DADAS13-mplasso
    logitMAD <- append(logitMAD,mean(abs(res)))
#    print(logitMAD)
#    print(logitcor)

    bm <- baggedModel(as.character(baggedformula),trainTime_ADAS13)
    bm$formula

#    mpsvm <- medianPredict(as.character(baggedformula),trainTime_ADAS13,testTime_ADAS13,predictType ="linear",type="SVM")
     mpsvm <- medianPredict(bm$formula,trainTime_ADAS13,testTime_ADAS13,predictType ="linear",type="SVM")
    
    plot(testTime_ADAS13$DADAS13~mpsvm$medianPredict)
    ct <- cor.test(testTime_ADAS13$DADAS13,mpsvm$medianPredict)
    logitcor <- append(logitcor,ct$estimate)
    res <- testTime_ADAS13$DADAS13-mpsvm$medianPredict
    logitMAD <- append(logitMAD,mean(abs(res)))

    mp <- medianPredict(as.character(baggedformula),trainTime_ADAS13,testTime_ADAS13,predictType ="linear",type="LM")
    mp$medianPredict <- (as.vector(mp$medianPredict)+as.vector(mplasso)+as.vector(mpsvm$medianPredict))/3.0
    names(mp$medianPredict) <- rownames(testTime_ADAS13)
...
#    testWithAD_s <- subset(testTadploe.norm,LastCompleteObs==1 & BDX==3)
...
#        testWithAD_s$timetolast_MMSE <- testWithAD_s$MMSE*log(timetolast)
#        testWithAD_s$timetolast_BDX <- testWithAD_s$BDX*log(timetolast)
#        mp <- medianPredict(as.character(baggedformula),trainTime_ADAS13,testWithAD_s,predictType ="linear",type="LM")
#        forcasttestPredicts <- cbind(forcasttestPredicts,mplasso+testWithAD_s$ADAS13)
#    }
...
#        testD3$timetolast_MMSE <- testD3$MMSE*log(timetolast)
#        testD3$timetolast_BDX <- testD3$BDX*log(timetolast)
#        mp <- medianPredict(as.character(baggedformula),trainTime_ADAS13,testD3,predictType ="linear",type="LM")
#        tmp <- mplasso+testD3$ADAS13
...
#write.csv(allforcasttestPredicts,file=paste(repp,"D2_ADAS13_AD.csv",sep="_"))
#write.csv(allD3predicts,file=paste(repp,"D3_ADAS13_AD.csv",sep="_"))
...
#  }
#}
#write.csv(mean50CI,file=paste(repp,"D2_ADAS13_AD_mean50CI.csv",sep="_"))
...
#  }
#}
#write.csv(mean50CI,file=paste(repp,"D3_ADAS13_AD_mean50CI.csv",sep="_"))




bp <- boxplot(alltestpredicts[,2]~rownames(alltestpredicts),plot=TRUE)
bpstatx <- bp$stats[3,]
...
barplot(rbind(logitMAD[3*(1:50)-2],logitMAD[3*(1:50)-1],logitMAD[3*(1:50)]),beside = TRUE)
