  ...
  ids_ca <- unique(as.character(CASESMCI$RID))
    ...
    if (compute)
    {
      for (n in 1:numberOfTrials)
      {
        ...
        sys_times <- rbind(sys_times,system.time(Time_ADAS13[[n]] <- FRESA.Model(DADAS13 ~ 1,singlecaseTime_ADAS13,filter.p.value = 0.025)))
      ...
      save(sys_times,file=paste("sys_times_Time_ADAS13_MCI",numberOfTrials,j,repp,".RDATA",sep="_"))
      save(Time_ADAS13,file=paste("Time_ADAS13_MCI",numberOfTrials,j,repp,".RDATA",sep="_"))
    }
    else
    {
      load(file=paste("Time_ADAS13_MCI",numberOfTrials,j,repp,".RDATA",sep="_"))
...
write.csv(allforcasttestPredicts,file=paste(repp,"D2_ADAS13_MCI.csv",sep="_"))
write.csv(allD3predicts,file=paste(repp,"D3_ADAS13_MCI.csv",sep="_"))
...
write.csv(mean50CI,file=paste(repp,"D2_ADAS13_MCI_mean50CI.csv",sep="_"))
...
write.csv(mean50CI,file=paste(repp,"D3_ADAS13_MCI_mean50CI.csv",sep="_"))
