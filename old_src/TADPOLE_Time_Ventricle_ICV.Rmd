...
for (j in 1:length(VISIT_TRAINCROSSMRI) )
{
  ...
  CASESAD <- rbind(CASESAD,cbind(V_ICV,timetolast,tmpss[,sampledcolumns]))
}

hist(CASESAD$V_ICV)
CASESAD <- CASESAD[complete.cases(CASESAD),]
CASESAD$ICV <- NULL
#CASESAD$Ventricle_ICV <- NULL
CASESAD$timetolast2 <- CASESAD$timetolast*CASESAD$timetolast
CASESAD$timetolast3 <- log(CASESAD$timetolast)
CASESAD$timetolast_MeanCV <- CASESAD$MeanCV*log(CASESAD$timetolast)
TADPOLE_predictors <- read.delim("PredictorsList.txt")
...
V_ICV_RankFeaturesRaw["Ventricle_ICV",]
```


```{r Modeling V_ICV time in AD}
compute = TRUE
numberofRepp = 10
numberofFolds = 5
numberOfTrials = 5
...
testWithAD$V_ICV <- numeric(nrow(testWithAD))
...
for ( repp in 1:numberofRepp)
{
  ...
  ids_ca <- unique(as.character(CASESAD$RID))
    ...
    trainTime_V_ICV <- subset(CASESAD,RID %in% sampleIDS_TimeV_ICV)
    testTime_V_ICV <- subset(CASESAD,!(RID %in% sampleIDS_TimeV_ICV))
    ...
    if (compute)
    {
      ...
      save(sys_times,file=paste("sys_times_Time_V_ICV3",numberOfTrials,j,repp,".RDATA",sep="_"))
      save(Time_V_ICV,file=paste("Time_V_ICV",numberOfTrials,j,repp,".RDATA",sep="_"))
    }
    else
    {
      load(file=paste("Time_V_ICV",numberOfTrials,j,repp,".RDATA",sep="_"))
...
#    mp <- medianPredict(as.character(baggedformula),trainTime_V_ICV,testTime_V_ICV,predictType ="linear",type="SVM")
    alltestpredicts <- rbind(alltestpredicts,cbind(testTime_V_ICV$V_ICV,mp$medianPredict))
    plot(testTime_V_ICV$V_ICV~mp$medianPredict,xlim=c(0,0.02),ylim=c(0,0.02))
    ...
    bm <- baggedModel(as.character(baggedformula),trainTime_V_ICV)
    bpr <- predict(bm$bagged.model,testTime_V_ICV)

    plot(testTime_V_ICV$V_ICV~bpr,xlim=c(0,0.02),ylim=c(0,0.02))
    ct <- cor.test(testTime_V_ICV$V_ICV,bpr)
    res <- testTime_V_ICV$V_ICV-bpr
...
#    Fullenet <- try(glmnet::cv.glmnet(as.matrix(trainTime_V_ICV[,-1]),as.vector(trainTime_V_ICV[,1]),family="gaussian"));
#    bpr <- predict(Fullenet,as.matrix(testTime_V_ICV[,-1]))
#    print(c(ct$estimate,mean(abs(res))))
...
write.csv(allforcasttestPredicts,file=paste(repp,"D2_V_ICV_AD.csv",sep="_"))
write.csv(allD3predicts,file=paste(repp,"D3_V_ICV_AD.csv",sep="_"))
...
write.csv(mean50CI,file=paste(repp,"D2_V_ICV_AD_mean50CI.csv",sep="_"))
...
write.csv(mean50CI,file=paste(repp,"D3_V_ICV_AD_mean50CI.csv",sep="_"))
...
bpstaty <- bp$stats[3,]
plot(bpstaty~bpstatx,main="All Tests",xlim=c(0,0.02),ylim=c(0,0.02))
