...
condition_data <- function() {
    ...
    TADPOLE_CROSSMRI <- read.delim("data/CompleMRIObs.txt",na.strings = c("NA","","#DIV/0!"))
    TADPOLE_predictors <- read.delim("data/PredictorsList.txt")
    D3_Set <- read.delim("data/D3Clean.txt",na.strings = c("NA","","#DIV/0!"))
    ...
    # cell 3 {r Adjusting for age and ICV}
